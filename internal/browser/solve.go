...
type solveDirectJob struct {
	...
	URL      string
	Ctx      context.Context
	Reply    chan *model.SolveDirectResp
	Enqueued time.Time
}
...
func (w *worker) runSolveDirect(job *solveDirectJob) (model.SolveDirectResp, error) {
	...
	// Counters for Turnstile solve detection
	noCheckboxCount := 0
	...
	for {
		...
		// Try to capture cookies from Storage API (includes HttpOnly cookies)
		...
		// Detect Turnstile auto-solve (checkbox disappears after click)
		if !checkboxFound {
			noCheckboxCount++
			if !lastClickTime.IsZero() && time.Since(lastClickTime) > 3*time.Second {
				noCheckboxAfterClickCount++
				if noCheckboxAfterClickCount >= 3 {
